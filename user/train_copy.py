...
class DirSelfPlayLatest:
    """
    Lightweight self-play provider that selects the latest checkpoint
    from a directory and builds an opponent using the provided partial.
    Does not depend on SaveHandler, so it's safe inside VecEnv workers.
    """
    def __init__(self, agent_partial: partial, ckpt_dir: str):
        self.agent_partial = agent_partial
        self.ckpt_dir = ckpt_dir
        self.env = None  # will be set by the caller after env creation

    def get_opponent(self) -> Agent:
        path = _latest_ckpt(self.ckpt_dir, "rl_model_")
        if path is None:
            # Fall back to a simple baseline if nothing to load
            opponent = ConstantAgent()
        else:
            opponent = self.agent_partial(file_path=path)
        # Initialize with current env
        if self.env is None:
            raise RuntimeError("DirSelfPlayLatest.env not set")
        opponent.get_env_info(self.env)
        return opponent


class DirSelfPlayRandom:
    """
    Lightweight self-play provider that randomly selects a checkpoint
    from a directory and builds an opponent using the provided partial.
    Does not depend on SaveHandler, so it's safe inside VecEnv workers.
    """
    def __init__(self, agent_partial: partial, ckpt_dir: str):
    ...
    def _pick_random(self) -> Optional[str]:
    ...
    def get_opponent(self) -> Agent:
        path = self._pick_random()
        if path is None:
            opponent = ConstantAgent()
        else:
            opponent = self.agent_partial(file_path=path)
        if self.env is None:
            raise RuntimeError("DirSelfPlayRandom.env not set")
        opponent.get_env_info(self.env)
        return opponent
...
def make_env(i: int,
    ...
    def _init():
        ...
        # Attach env reference so DirSelfPlay providers can init opponents
        sp.env = env
